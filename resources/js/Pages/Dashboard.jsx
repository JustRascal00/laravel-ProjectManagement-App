        ...
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 grid grid-cols-3 gap-2">
          ...
          <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div className="p-6 text-gray-900 dark:text-gray-100">
              ...
              <p className="text-xl mt-4">
                <span className="mr-2">{myCompltedTasks}</span>/
                <span className="ml-2">{totalCompltedTasks}</span>
              </p>
            </div>
          </div>
        </div>
              ...
              <table className="mt-3 w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {activeTasks.data.map((task) => (
                    <tr key={task.id}>
                      <td px-3 className="px-3 py-2">
                        {task.id}
                      </td>
                      <td
                        px-3
                        className="px-3 py-2 text-white font-bold hover:underline"
                      >
                        <Link href={route("project.show", task.project.id)}>
                          {task.project.name}
                        </Link>
                      </td>
                      <td px-3 className="px-3 py-2 text-white hover:underline">
